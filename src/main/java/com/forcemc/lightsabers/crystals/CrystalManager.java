...
public class CrystalManager {
    ...
    private final String[][] defaultCrystals = {
    ...
    public void load() {
        ...
        if (lightsabers.getConfigurationManager().isFresh()) {
            for (String[] defaultCrystal : defaultCrystals) {
                if (lightsabers.getConfigurationManager().getCrystals().contains("crystals." + defaultCrystal[2]))
                    continue;
    ...
    public boolean isDefaultCrystal(Crystal crystal) {
        return Arrays.stream(defaultCrystals).anyMatch(string -> string[2].equalsIgnoreCase(crystal.getUuid().toString()));
    }
